...
cap = cv2.VideoCapture("https://atcs-dishub.bandung.go.id:1990/Gardujati/main_stream.m3u8") 
...
while True:
    ...
    #boxes and lines
    zona_menunggu = [
        [int(w * 0.01), int(h * 0.45), int(w * 0.1), int(h * 0.65)],
        [int(w * 0.53), int(h * 0.05), int(w * 0.67), int(h * 0.25)]
    ]
    
    x_garis1, y_garis1 = int(w * 0.1), int(h * 0.55)  
    x_garis2, y_garis2 = int(w * 0.58), int(h * 0.25)  
    ...
    cv2.putText(gambar_final, "GARIS KENDARAAN", (x_garis2 - 10, y_garis2 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 165, 255), 1)
